# integrations/silhouette/nox_save_dialog.py
"""
Silhouette-specific Save Dialog for NOX VFX
Handles Silhouette-specific save options and workflow
"""
...
class NOXSaveDialogSilhouette(QDialog):
    """
    Silhouette-specific save dialog with Silhouette-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Silhouette-specific save UI"""
        self.setWindowTitle("NOX Save - Silhouette")
        ...
        # Silhouette-specific save options
        layout.addWidget(self._create_silhouette_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.sfx", "*.shape"])
    ...
    def _create_silhouette_save_options(self) -> QWidget:
        """Create Silhouette-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Silhouette Save Options")
        ...
        # Silhouette-specific options
        silhouette_options_layout = QHBoxLayout()
        silhouette_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.save_project_only_checkbox = QCheckBox("Save project only (no render)")
        self.save_project_only_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.save_project_only_checkbox)
        
        self.include_render_settings_checkbox = QCheckBox("Include render settings")
        self.include_render_settings_checkbox.setChecked(True)
        checkbox_layout.addWidget(self.include_render_settings_checkbox)
        ...
        silhouette_options_layout.addLayout(checkbox_layout)
        silhouette_options_layout.addStretch()
        
        save_layout.addLayout(silhouette_options_layout)
    ...
    def _populate_file_tree(self, directory: str):
        ...
        try:
            ...
            # List Silhouette files
            for entry in os.scandir(directory):
                if entry.is_file():
                    ext = os.path.splitext(entry.name)[1]
                    if ext not in [".sfx", ".shape"]:
                        continue
    ...
    def get_result(self) -> Dict[str, Any]:
        """Get Silhouette save dialog result"""
        return {
            'file_path': self.selected_file,
            'suffix': self.suffix_input.text(),
            'version': self.version_combo.currentText(),
            'auto_increment': self.auto_increment_checkbox.isChecked(),
            'save_project_only': self.save_project_only_checkbox.isChecked(),
            'include_render_settings': self.include_render_settings_checkbox.isChecked(),
