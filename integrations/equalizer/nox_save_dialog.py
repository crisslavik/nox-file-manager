# integrations/equalizer/nox_save_dialog.py
"""
Equalizer-specific Save Dialog for NOX VFX
Handles Equalizer-specific save options and workflow
"""
...
class NOXSaveDialogEqualizer(QDialog):
    """
    Equalizer-specific save dialog with Equalizer-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Equalizer-specific save UI"""
        self.setWindowTitle("NOX Save - Equalizer")
        ...
        # Equalizer-specific save options
        layout.addWidget(self._create_equalizer_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.eqz"])
    ...
    def _create_equalizer_save_options(self) -> QWidget:
        """Create Equalizer-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Equalizer Save Options")
        ...
        # Equalizer-specific options
        equalizer_options_layout = QHBoxLayout()
        equalizer_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.save_audio_processing_checkbox = QCheckBox("Save audio processing settings")
        self.save_audio_processing_checkbox.setChecked(True)
        checkbox_layout.addWidget(self.save_audio_processing_checkbox)
        
        self.include_presets_checkbox = QCheckBox("Include presets in project")
        self.include_presets_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.include_presets_checkbox)
        ...
        equalizer_options_layout.addLayout(checkbox_layout)
        equalizer_options_layout.addStretch()
        
        save_layout.addLayout(equalizer_options_layout)
    ...
    def get_result(self) -> Dict[str, Any]:
        """Get Equalizer save dialog result"""
        return {
            'file_path': self.selected_file,
            'suffix': self.suffix_input.text(),
            'version': self.version_combo.currentText(),
            'auto_increment': self.auto_increment_checkbox.isChecked(),
            'save_audio_processing': self.save_audio_processing_checkbox.isChecked(),
            'include_presets': self.include_presets_checkbox.isChecked(),
