# integrations/maya/nox_save_dialog.py
"""
Maya-specific Save Dialog for NOX VFX
Handles Maya-specific save options and workflow
"""
...
class NOXSaveDialogMaya(QDialog):
    """
    Maya-specific save dialog with Maya-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Maya-specific save UI"""
        self.setWindowTitle("NOX Save - Maya")
        ...
        # Maya-specific save options
        layout.addWidget(self._create_maya_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.ma", "*.mb"])
    ...
    def _create_maya_save_options(self) -> QWidget:
        """Create Maya-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Maya Save Options")
        ...
        save_layout.addLayout(filetype_layout)
        
        # Maya-specific options
        maya_options_layout = QHBoxLayout()
        maya_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.keep_history_checkbox = QCheckBox("Keep history in file")
        self.keep_history_checkbox.setChecked(True)
        checkbox_layout.addWidget(self.keep_history_checkbox)
        
        self.optimize_scene_checkbox = QCheckBox("Optimize scene size")
        checkbox_layout.addWidget(self.optimize_scene_checkbox)
        
        maya_options_layout.addLayout(checkbox_layout)
        maya_options_layout.addStretch()
        
        save_layout.addLayout(maya_options_layout)
    ...
    def get_result(self) -> Dict[str, Any]:
        """Get Maya save dialog result"""
        return {
            'file_path': self.selected_file,
            'suffix': self.suffix_input.text(),
            'version': self.version_combo.currentText(),
            'auto_increment': self.auto_increment_checkbox.isChecked(),
            'file_type': 'ma' if self.filetype_combo.currentIndex() == 0 else 'mb',
            'keep_history': self.keep_history_checkbox.isChecked(),
            'optimize_scene': self.optimize_scene_checkbox.isChecked(),
