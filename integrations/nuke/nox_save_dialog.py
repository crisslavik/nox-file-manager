# integrations/nuke/nox_save_dialog.py
"""
Nuke-specific Save Dialog for NOX VFX
Handles Nuke-specific save options and workflow
"""
...
class NOXSaveDialogNuke(QDialog):
    """
    Nuke-specific save dialog with Nuke-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Nuke-specific save UI"""
        self.setWindowTitle("NOX Save - Nuke")
        ...
        # Nuke-specific save options
        layout.addWidget(self._create_nuke_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.nk", "*.nknc"])
    ...
    def _create_nuke_save_options(self) -> QWidget:
        """Create Nuke-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Nuke Save Options")
        ...
        nuke_options_layout = QHBoxLayout()
        nuke_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.save_comp_script_checkbox = QCheckBox("Save comp script only")
        self.save_comp_script_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.save_comp_script_checkbox)
        
        self.compress_script_checkbox = QCheckBox("Compress script (.nknc)")
        self.compress_script_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.compress_script_checkbox)
        ...
        nuke_options_layout.addLayout(checkbox_layout)
        nuke_options_layout.addStretch()
        
        save_layout.addLayout(nuke_options_layout)
    ...
    def _connect_signals(self):
        ...
        self.compress_script_checkbox.toggled.connect(self._on_compress_toggled)
    ...
    def _on_validate_clicked(self):
        ...
        # In a real implementation, validate permissions, disk space, etc.
    ...
    def _on_save_clicked(self):
        ...
        # Accept dialog (parent will handle the actual save)
        self.accept()
    ...
    def _on_compress_toggled(self, checked: bool):
        """Handle compress script checkbox toggle"""
        if checked:
            # Update filter to show .nknc files
            self.filter_combo.clear()
            self.filter_combo.addItems(["All Files", "*.nknc", "*.nk"])
            self.filter_combo.setCurrentIndex(1)
        else:
            # Update filter to show .nk files
            self.filter_combo.clear()
            self.filter_combo.addItems(["All Files", "*.nk", "*.nknc"])
            self.filter_combo.setCurrentIndex(1)
    
    def get_result(self) -> Dict[str, Any]:
        """Get Nuke save dialog result"""
        return {
            'file_path': self.selected_file,
            'suffix': self.suffix_input.text(),
            'version': self.version_combo.currentText(),
            'auto_increment': self.auto_increment_checkbox.isChecked(),
            'save_comp_script_only': self.save_comp_script_checkbox.isChecked(),
            'compress_script': self.compress_script_checkbox.isChecked(),
