# integrations/mocha/nox_save_dialog.py
"""
Mocha-specific Save Dialog for NOX VFX
Handles Mocha-specific save options and workflow
"""
...
class NOXSaveDialogMocha(QDialog):
    """
    Mocha-specific save dialog with Mocha-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Mocha-specific save UI"""
        self.setWindowTitle("NOX Save - Mocha")
        ...
        # Mocha-specific save options
        layout.addWidget(self._create_mocha_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.mocha", "*.project"])
    ...
    def _create_mocha_save_options(self) -> QWidget:
        """Create Mocha-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Mocha Save Options")
        ...
        mocha_options_layout = QHBoxLayout()
        mocha_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.save_tracking_data_checkbox = QCheckBox("Save tracking data only")
        self.save_tracking_data_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.save_tracking_data_checkbox)
        
        self.include_keyframes_checkbox = QCheckBox("Include keyframes")
        self.include_keyframes_checkbox.setChecked(True)
        checkbox_layout.addWidget(self.include_keyframes_checkbox)
        ...
        mocha_options_layout.addLayout(checkbox_layout)
        mocha_options_layout.addStretch()
        
        save_layout.addLayout(mocha_options_layout)
    ...
    def get_result(self) -> Dict[str, Any]:
        """Get Mocha save dialog result"""
        return {
            'file_path': self.selected_file,
            'suffix': self.suffix_input.text(),
            'version': self.version_combo.currentText(),
            'auto_increment': self.auto_increment_checkbox.isChecked(),
            'save_tracking_data_only': self.save_tracking_data_checkbox.isChecked(),
            'include_keyframes': self.include_keyframes_checkbox.isChecked(),
