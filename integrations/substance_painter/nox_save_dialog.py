# integrations/substance_painter/nox_save_dialog.py
"""
Substance Painter-specific Save Dialog for NOX VFX
Handles Substance Painter-specific save options and workflow
"""
...
class NOXSaveDialogSubstancePainter(QDialog):
    """
    Substance Painter-specific save dialog with Substance Painter-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Substance Painter-specific save UI"""
        self.setWindowTitle("NOX Save - Substance Painter")
        ...
        # Substance Painter-specific save options
        layout.addWidget(self._create_substance_painter_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.spp"])
    ...
    def _create_substance_painter_save_options(self) -> QWidget:
        """Create Substance Painter-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Substance Painter Save Options")
        ...
        # Substance Painter-specific options
        substance_options_layout = QHBoxLayout()
        substance_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.export_textures_checkbox = QCheckBox("Export textures on save")
        self.export_textures_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.export_textures_checkbox)
        
        self.save_mesh_checkbox = QCheckBox("Save mesh with project")
        self.save_mesh_checkbox.setChecked(True)
        checkbox_layout.addWidget(self.save_mesh_checkbox)
        ...
        substance_options_layout.addLayout(checkbox_layout)
        substance_options_layout.addStretch()
        
        save_layout.addLayout(substance_options_layout)
    ...
    def get_result(self) -> Dict[str, Any]:
        """Get Substance Painter save dialog result"""
        return {
            'file_path': self.selected_file,
            'suffix': self.suffix_input.text(),
            'version': self.version_combo.currentText(),
            'auto_increment': self.auto_increment_checkbox.isChecked(),
            'export_textures': self.export_textures_checkbox.isChecked(),
            'save_mesh': self.save_mesh_checkbox.isChecked(),
