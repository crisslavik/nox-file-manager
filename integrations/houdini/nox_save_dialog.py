# integrations/houdini/nox_save_dialog.py
"""
Houdini-specific Save Dialog for NOX VFX
Handles Houdini-specific save options and workflow
"""
...
class NOXSaveDialogHoudini(QDialog):
    """
    Houdini-specific save dialog with Houdini-specific options
    """
    ...
    def _setup_ui(self):
        """Setup the Houdini-specific save UI"""
        self.setWindowTitle("NOX Save - Houdini")
        ...
        # Houdini-specific save options
        layout.addWidget(self._create_houdini_save_options())
    ...
    def _create_browser_section(self) -> QWidget:
        ...
        self.filter_combo.addItems(["All Files", "*.hip", "*.hipnc"])
    ...
    def _create_houdini_save_options(self) -> QWidget:
        """Create Houdini-specific save options"""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        
        save_group = QGroupBox("Houdini Save Options")
        ...
        houdini_options_layout = QHBoxLayout()
        houdini_options_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.compress_file_checkbox = QCheckBox("Compress scene file (.hipnc)")
        self.compress_file_checkbox.setChecked(False)
        checkbox_layout.addWidget(self.compress_file_checkbox)
        
        self.save_with_external_references_checkbox = QCheckBox("Save with external references")
        self.save_with_external_references_checkbox.setChecked(True)
        checkbox_layout.addWidget(self.save_with_external_references_checkbox)
        ...
        houdini_options_layout.addLayout(checkbox_layout)
        houdini_options_layout.addStretch()
        
        save_layout.addLayout(houdini_options_layout)
    ...
    def _connect_signals(self):
        ...
        self.compress_file_checkbox.toggled.connect(self._on_compress_toggled)
    ...
    def _on_compress_toggled(self, checked: bool):
        """Handle compress file checkbox toggle"""
        if checked:
            self.filter_combo.clear()
            self.filter_combo.addItems(["All Files", "*.hipnc", "*.hip"])
            self.filter_combo.setCurrentIndex(1)
        else:
            self.filter_combo.clear()
            self.filter_combo.addItems(["All Files", "*.hip", "*.hipnc"])
    ...
    def get_result(self) -> Dict[str, Any]:
        """Get Houdini save dialog result"""
        return {
            ...
            'save_with_external_references': self.save_with_external_references_checkbox.isChecked(),
